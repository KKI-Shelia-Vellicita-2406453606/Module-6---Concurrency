...
The handle_connection function was refactored to eliminate code duplication by utilizing a match expression to assign the status_line and filename based on the request line. I also introduced a '/sleep' route using 'thread::sleep(Duration::from_secs(10))' to simulate a slow processing task, which revealed a critical performance bottleneck because the server is currently single-threaded, it can only process one request at a time. This results in a blocking behavior where a long-running request prevents all other users from accessing the server until the first task completes. This exercise highlights the necessity of concurrency, such as a thread pool—to handle multiple simultaneous connections efficiently.